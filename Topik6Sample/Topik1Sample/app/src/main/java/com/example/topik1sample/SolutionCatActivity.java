...
public class SolutionCatActivity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        prob_set = intent.getStringExtra(PROB_SET);
        ...
        getData("http://172.30.1.31:5000/topik1_solution/"); // visual code 상에서 구현
        ...
        Log.d("prob_round",prob_set);

        RequestBody formbody = new FormBody.Builder().add("prob_num",prob_num).add("prob_round",prob_set).build();

        Request request = new Request.Builder().url("http://172.30.1.31:5000/topik1_solution/").post(formbody).build();
        okHttpClient.newCall(request).enqueue(new Callback(){
            @Override
            public void onFailure(@NotNull okhttp3.Call call, @NotNull IOException e) {
                runOnUiThread(new Runnable() {
                    public void run() {
                        Toast.makeText(SolutionCatActivity.this, "network error...!", Toast.LENGTH_SHORT).show();
                    }
                });
            }
